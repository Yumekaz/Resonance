...
func validateSchemaContract(ctx context.Context, q queryer) error {
	columns := []string{
		...
		"tracks|artist_credit|text|false|", "tracks|album_title|text|false|", "tracks|album_artist_credit|text|false|", "tracks|track_number|integer|false|", "tracks|disc_number|integer|false|", "tracks|release_year|integer|false|", "tracks|genre|text|false|", "tracks|title_source|text|false|",
...
func validateLibraryContract(ctx context.Context, q queryer) error {
	columns := []string{
		"library_roots|id|uuid|true|", "library_roots|name|text|true|", "library_roots|canonical_path|text|true|", "library_roots|path_key|text|true|", "library_roots|enabled|boolean|true|true", "library_roots|created_at|timestamp with time zone|true|now()",
	...
	if err := matchContract(ctx, q, `SELECT c.relname || '|' || k.conname || '|' || pg_get_constraintdef(k.oid)
		...
		AND k.conname IN ('library_roots_pkey','library_roots_name_check','library_roots_canonical_path_check','library_roots_path_key_key','tracks_track_number_check','tracks_disc_number_check','tracks_release_year_check','tracks_title_source_check','media_objects_artwork_sha256_check','media_locations_root_id_fkey','media_locations_root_pair','scan_runs_pkey','scan_runs_root_id_fkey','scan_runs_status_check','scan_errors_pkey','scan_errors_run_id_fkey','scan_errors_relative_path_check','scan_errors_code_check')
		AND k.convalidated AND (k.conindid=0 OR (i.indisvalid AND i.indisready))`, constraints); err != nil {
		return err
	}
	return matchContract(ctx, q, `SELECT c.relname || '|' || x.relname || '|' || i.indisunique::text || '|' || i.indnkeyatts::text || '|' || pg_get_indexdef(i.indexrelid,1,true) || '|' || coalesce(pg_get_indexdef(i.indexrelid,2,true),'') || '|' || coalesce(pg_get_expr(i.indpred,i.indrelid),'') || '|' || i.indoption::text
		...
		WHERE x.oid IN (to_regclass('media_locations_root_relative_idx'),to_regclass('scan_runs_root_started_idx'),to_regclass('scan_errors_run_idx'))
		AND i.indisvalid AND i.indisready AND i.indnatts=i.indnkeyatts AND i.indexprs IS NULL AND am.amname='btree'`, []string{
		"media_locations|media_locations_root_relative_idx|true|2|root_id|relative_path|(root_id IS NOT NULL)|0 0",
